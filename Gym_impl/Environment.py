...
class Environment(gym.Env):
    ...
    def step(self, action):
        ...
        for ix, device in enumerate(self.dList):
            ...
            if device.transmit_time < 1 : #device finished transmitting to its assigned receiver
                if np.random.random() < self.Para.transmit_duty: #to not have everyone transmitting all the time
    ...
    def get_state(self):
        return [(tx.id, list(self.dList[tx.rid].position)+list(tx.position)+[tx.power*tx.Pmax]) if tx.rid is not None \
            else (tx.id, list((0,0))+list((0,0))+[0]) \
                for tx in self.dList ]
    ...
    def compute_SINR(self, D, shadowing=False, fastfading=False):
        ...
        Tx_over_Rx = 6 + 20*np.log10(1+D/self.Para.Rbp)*(1+(D>self.Para.Rbp).astype(int)) # + self.Para.Lbp
    ...
    def objective(self):
        ...
        for j, dj in enumerate(self.dList):
            for i, di in enumerate(self.dList[j:]):
                D[j, j+i] = np.linalg.norm(dj.position*self.Para.side_length-di.position*self.Para.side_length)
                D[j+i, j] = D[j, j+i]
